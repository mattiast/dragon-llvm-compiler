...
type Var = String
data LValue = LVar Var | LArr LValue Expr deriving (Eq, Ord)
data Type = TInt | TFloat | TChar | TBool | TArr Type Int deriving (Eq, Ord)
...
data Decl = Decl Type Var deriving (Eq, Ord, Show)

data Stmt = SAssign LValue Expr
          | SBlock [Decl] [Stmt]
	      | SIf Expr Stmt Stmt
	      | SWhile Expr Stmt
          | SDoWhile Expr Stmt
...
extractLValue (LArr lv e1) = let (v,inds) = extractLValue lv
        in (v,inds ++ [e1])
